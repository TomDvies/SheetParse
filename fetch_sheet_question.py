...
def fetch_sheet(sheetarr) -> str:
    ...
    fileid = get_base64(sheetarr[0])  # url -> b64 to avoid collisions
...
def get_start_end(doc, q, course, formats):
    ...
    for x in range(len(doc)):
        ...
        starts = findall(text, startstr)+ findall(text,starredstartstr)
        if text.startswith(startstr.strip()) or text.startswith(starredstartstr.strip()):
            starts += [0]
        ends = findall(text, endstr) + findall(text,starredendstr)
        # print(startstr)
        if text.startswith(endstr.strip()) or text.startswith(starredendstr.strip()):
            ends += [0]
        if starts:
            start = [starts[0], x]
        if ends:
